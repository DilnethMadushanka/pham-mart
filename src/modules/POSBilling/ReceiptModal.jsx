...
export default function ReceiptModal({ txn, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/55 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-sky-100 overflow-hidden depth-card">
        
        {/* Modal Top Actions */}
        <div className="p-4 border-b border-sky-100 bg-gradient-to-r from-sky-50 to-blue-50/70 flex justify-between items-center no-print">
          <div className="flex items-center space-x-2">
            <div className="p-1.5 rounded-full bg-emerald-100 text-emerald-600"><CheckCircle className="w-4 h-4" /></div>
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-between items-center no-print">
          <button
            onClick={handlePrint}
            className="flex items-center space-x-1.5 px-4 py-2 bg-slate-800 hover:bg-slate-900 text-white rounded-2xl font-bold text-xs shadow-md transition-all hover:-translate-y-0.5 cursor-pointer"
          >
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-bold text-xs rounded-2xl shadow-lg shadow-sky-500/30 hover:-translate-y-0.5 transition-all cursor-pointer"
          >
            Done & Next Sale
          </button>
        </div>
