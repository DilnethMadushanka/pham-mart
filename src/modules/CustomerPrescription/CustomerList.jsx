...
export default function CustomerList({ customers, setCustomers, prescriptions, addAuditLog }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Top Header */}
      <div className="bg-white p-6 rounded-2xl border border-sky-100 shadow-xs flex justify-between items-center">
        <div>
        ...
        <button
          ...
          className="flex items-center space-x-1.5 px-4 py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs rounded-xl shadow-xs cursor-pointer"
        >
      ...
      <div className="relative max-w-md">
        ...
        <input 
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-sky-500 outline-hidden"
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredCustomers.map((cust) => {
          ...
          return (
            <div key={cust.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs hover:border-sky-300 transition-all space-y-3">
              <div className="flex justify-between items-start">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-full bg-sky-100 text-sky-800 font-bold flex items-center justify-center text-sm border border-sky-300">
              ...
              <div className="pt-2 border-t border-slate-100 flex justify-between items-center text-xs text-slate-500 font-medium">
                <span>Linked Prescriptions: <strong className="text-sky-700">{rxCount}</strong></span>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-5 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleAddCustomer} className="space-y-3 text-xs">
              ...
              <div className="pt-2 flex justify-end space-x-2">
                <button type="button" onClick={()=>setIsAddModalOpen(false)} className="px-4 py-2 bg-slate-100 font-bold rounded-xl cursor-pointer">Cancel</button>
                <button type="submit" className="px-5 py-2 bg-sky-600 text-white font-bold rounded-xl shadow-xs cursor-pointer">Save Customer Profile</button>
              </div>
            </form>
          </div>
        </div>
